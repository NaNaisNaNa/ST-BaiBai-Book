...
export const RESUMMARY2_MACROS: PromptMacro[] = [
  ...
  { token: '{{target}}', desc: '目标字数(按输入规模自动算出)' },
];
...
export const RESUMMARY_PROMPT = `你是剧情压缩助手。下面是若干段按时间先后排列的剧情摘要,请把它们压缩为一段信息密度极高、连贯的上层摘要(300-500 字),**只输出一个 JSON 对象**。
...
【输出要求】
- 篇幅 300-500 字;严格按事件发生的日期先后顺序,串联因果关系,形成一篇连贯的微型故事。
...
/**
 * 二次总结目标字数:参与内容总字数 × 系数(0.4~0.6 取中值 0.5),作为动态篇幅下限。
 * 钳一个保底下限,避免输入很短时目标过小失去意义。
 */
export function resummary2Target(contentLen: number): number {
  return Math.max(800, Math.round(contentLen * 0.5));
}

/**
 * 构造总结提示词。按层级分两套:
 *  - level<=1(L0 叶子 → L1):普通总结,固定 300-500 字 → RESUMMARY_PROMPT。
 *  - level>=2(L1+ → 更上层):二次总结,目标字数随输入规模动态给 → RESUMMARY2_PROMPT。
 * 各自优先用对应的自定义模板,空则回退内置。
 */
